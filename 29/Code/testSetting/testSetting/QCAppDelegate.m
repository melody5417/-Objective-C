...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    NSDictionary    *dictDefault = [self defaultSetting];
    if (dictDefault) {
        [[NSUserDefaults standardUserDefaults] registerDefaults:dictDefault];
...
{
    NSArray             *arrContent  = nil;
    NSDictionary        *dictSetting = nil;
    NSMutableDictionary *dictDefault = [NSMutableDictionary dictionaryWithCapacity:0];
    NSURL               *settingsUrl = [NSURL fileURLWithPath:[[NSBundle mainBundle]
    ...
    dictSetting = [NSDictionary dictionaryWithContentsOfURL:settingsUrl];
    if (!dictSetting) {
        return dictDefault;
    }
    
    arrContent  = [dictSetting objectForKey:@"PreferenceSpecifiers"];
    NSString    *strKey     = nil;
    NSString    *strValue   = nil;
    for (id oneObj in arrContent) {
        if (![oneObj isKindOfClass:[NSDictionary class]]) {
            continue;
        }
        
        strKey      = [(NSDictionary*)oneObj objectForKey:@"Key"];
        strValue    = [(NSDictionary*)oneObj objectForKey:@"DefaultValue"];
        if (strValue && strKey) {
            [dictDefault setObject:strValue
                            forKey:strKey];
        }
    }
